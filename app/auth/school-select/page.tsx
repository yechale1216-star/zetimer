...
export default function SchoolSelectPage() {
  ...
  const handleSelectSchool = async (schoolId: string) => {
    ...
    if (success) {
      ...
      // Get the updated user from AuthService (which was updated by switchSchool -> validateSession)
      const updatedUser = authService.getCurrentUser()
      const role = updatedUser?.role

      console.log(`[SchoolSelect] Switched successfully. New role: ${role}`)

      if (role === "admin" || role === "school_admin") {
        if (updatedUser?.onboardingCompleted === false) {
