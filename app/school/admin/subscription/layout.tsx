...
export default function SubscriptionLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-full space-y-6 p-4 md:p-6">
      ...
      {/* Scrollable tab strip — full width, no wrapping, swipeable on mobile */}
      <div className="w-full overflow-x-auto scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
        <div className="flex space-x-1 rounded-xl bg-secondary/20 p-1 w-max min-w-full">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center gap-2 px-3 py-2.5 text-sm font-medium rounded-lg transition-all whitespace-nowrap flex-shrink-0",
                  isActive
