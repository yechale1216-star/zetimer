...
import { User, Mail, Phone, Shield, BookOpen } from 'lucide-react'
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-6 space-y-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-indigo-500/15 flex items-center justify-center">
            <span className="text-2xl font-black text-indigo-600">{user?.name?.charAt(0)?.toUpperCase()}</span>
          </div>
          <div>
            <p className="text-xl font-black text-foreground">{user?.name}</p>
            <div className="flex items-center gap-2 mt-1">
              <div className="w-2 h-2 rounded-full bg-indigo-500" />
              <span className="text-sm font-semibold text-indigo-600">Student Registration Officer (Registrar)</span>
            </div>
          </div>
        </div>
        <div className="space-y-3 pt-4 border-t border-border/40">
          {[
            ...
            { icon: Shield, label: 'Role', value: 'Registrar' },
            { icon: BookOpen, label: 'School ID', value: user?.schoolId || 'N/A' },
          ].map(f => (
